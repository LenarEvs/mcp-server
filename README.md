...
1. Откройте файл конфигурации Claude Desktop:
   ...
   Если файла нет — его можно создать через UI: **Settings → Developer → Edit Config**
   (кнопка откроет/создаст нужный файл и покажет его реальный путь).
2. Добавьте в `mcpServers` содержимое из
   [claude_desktop_config_example.json](claude_desktop_config_example.json), заменив пути
   на свои:
   - `args[0]` — абсолютный путь до `dist/server.js` внутри `mcp-server`;
   - `env.PROJECT_ROOT` — абсолютный путь до проекта, с которым должен работать сервер
     (можно не указывать, если нужно работать с самим `mcp-server`).
   ```json
   {
     "mcpServers": {
       "project-helper": {
   ...
   ```
3. Полностью перезапустите Claude Desktop — закрытия окна недостаточно, процесс
   продолжает работать в фоне со старым конфигом. Завершите его через иконку в трее
   (Quit/Exit) либо, если это не помогает, через Диспетчер задач (Task Manager).
4. Проверьте подключение в **Settings → Developer**: там должен появиться сервер
   `project-helper`. Если список пуст («No servers added»), значит конфиг не был
   перечитан — повторите шаг 3.
5. Дополнительно можно спросить в чате что-то, требующее один из инструментов
   (например, «покажи файлы в проекте») — Claude должен предложить вызвать `list_files`.
