...
---
Почини следующие замечания в проекте mcp-server, не меняя логику тулов:

1. dist/server.js трекается в git, хотя .gitignore исключает "dist".
   Выполни: git rm --cached dist/server.js
   (файл остаётся на диске, просто перестаёт трекаться; npm run build пересоберёт его заново)

2. Корневой server.js — это старый skeleton без тулов, оставшийся от
   первого шага разработки. Он не используется ни одним npm-скриптом
   (start использует src/server.ts, start:dist — dist/server.js) и
   расходится с актуальной реализацией. Удали файл server.js из корня
   и из git: git rm server.js

3. Папка docs/ пустая и не используется — удали её, если в ней
   действительно нет файлов (проверь перед удалением).

4. Создай файл report/demo.md с шаблоном таблицы (пустой, я заполню
   сам после реального тестирования в IDE) со столбцами:
   | № | Запрос пользователя | Ожидаемый tool | Подтверждение (лог/скрин) |
   Добавь под таблицей место для вставки фрагментов stderr-логов.

5. Обнови README.md: в разделе "Проверка"/"Ссылки на код" добавь
   пункт со ссылкой на report/demo.md как место хранения
   подтверждений вызовов тулов из IDE.

После изменений покажи git status и git diff --stat, не делай commit.
